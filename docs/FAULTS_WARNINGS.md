...
| **RTC_FAULT** | **sensors_task**: when RTC reads/flag handling fail or the RTC reports invalid time (`VLF`) | **sensors_task**: when RTC read/clear succeeds; or retry: RTC re-probe success |
| **LOW_BATT** | **adc_frames**: per-frame when SOC or voltage below config thresholds | **adc_frames**: when above thresholds |
| **BATT_WARN** | **sensors_task**: when fuel-gauge init or runtime reads fail, so battery telemetry is unavailable | **sensors_task**: when fuel-gauge communication recovers |
...
| **IMU_WARN** | **sensors_task** / **adc_frames**: runtime IMU read failures invalidate IMU data and set the warning until reads recover | Cleared on successful IMU reads |

## RETRY (short press in FAULT)

- Calls `logger_begin()` (SD remount). On success, clears **SD_MOUNT_FAIL** and **SD_WRITE_FAIL**.
- Calls `sensors_request_retry_probe()`. The sensors task re-runs `begin` for IMU, RTC, and the fuel gauge; on success it clears **IMU_FAULT**, **RTC_FAULT**, and **BATT_WARN** as the devices recover.
